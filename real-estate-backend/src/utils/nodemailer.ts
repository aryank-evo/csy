...
// Lead notification email interface
...
export const sendLeadNotificationEmail = async (
  ...
  try {
    ...
    if (data.propertyTitle) propertyDetails.push(`<strong>Property:</strong> ${data.propertyTitle}`);
    if (data.propertyPrice) propertyDetails.push(`<strong>Price:</strong> ${data.propertyPrice}`);
    if (data.propertyLocation) propertyDetails.push(`<strong>Location:</strong> ${data.propertyLocation}`);
    if (data.propertyType) propertyDetails.push(`<strong>Type:</strong> ${data.propertyType}`);
    ...
    return {
      success: false,
      message: `Failed to send lead notification email: ${(error as Error).message}`,
    };
  }
};
